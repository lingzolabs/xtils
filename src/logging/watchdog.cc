...
double MeanForArray(const uint64_t array[], size_t size) {
  ...
  return static_cast<double>(total * 1.0 / size);
}

struct timespec ToPosixTimespec(TimeMillis ms) {
  struct timespec ts;
...
bool ReadProcStat(int fd, ProcStat* out) {
  ...
  while (c_pos < sizeof(c) - 1) {
    ssize_t rd = read(fd, c + c_pos, sizeof(c) - c_pos);
    if (rd < 0) {
      LogW("Failed to read stat file to enforce resource limits.");
      return false;
    }
    if (rd == 0) break;
...
void Watchdog::ThreadMain() {
  ...
  xtils::ScopedFile stat_fd(open("/proc/self/stat", O_RDONLY));
  if (!stat_fd) {
    // ELOG("Failed to open stat file to enforce resource limits.");
    return;
  }
  ...
  for (;;) {
    ...
    if (tid_to_kill) SerializeLogsAndKillThread(tid_to_kill, crash_reason);

    // Check CPU and memory guardrails (if enabled).
    lseek(stat_fd.get(), 0, SEEK_SET);
    ProcStat stat;
    if (!ReadProcStat(stat_fd.get(), &stat)) continue;
    uint64_t cpu_time = stat.utime + stat.stime;
    uint64_t rss_bytes = static_cast<uint64_t>(stat.rss_pages) * 4096;
    ...
    if (threshold_exceeded) SerializeLogsAndKillThread(getpid(), crash_reason);
  }
}
...
void Watchdog::SerializeLogsAndKillThread(int tid,
  ...
  // In the case of a CHECK/FATAL this is done in logging.h.
  ...
  if (disable_kill_failsafe_for_testing_) return;
...
bool Watchdog::CheckMemory_Locked(uint64_t rss_bytes) {
  if (memory_limit_bytes_ == 0) return false;
...
bool Watchdog::CheckCpu_Locked(uint64_t cpu_time) {
  if (cpu_limit_percentage_ == 0) return false;
  ...
  if (cpu_window_time_ticks_.Push(cpu_time)) {
    ...
    if (percentage > cpu_limit_percentage_) {
      // ELOG("CPU watchdog trigger. %f%% CPU use is above the %"
      // PRIu32
      //               "%% CPU limit.",
...
Watchdog::Timer::Timer(Watchdog* watchdog, uint32_t ms,
                       WatchdogCrashReason crash_reason)
    : watchdog_(watchdog) {
  if (!ms) return;  // No-op timer created when the watchdog is disabled.
...
Watchdog::Timer::~Timer() {
  if (timer_data_.deadline.count()) watchdog_->RemoveFatalTimer(timer_data_);
}
