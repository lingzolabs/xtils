...
inline uint8_t DecodeChar(char c) {
  if (c < kMinDecChar || c > kMaxDecChar) return kX;
...
ssize_t Base64Encode(const void* src, size_t src_size, char* dst,
  ...
  if (dst_size < padded_dst_size) return -1;  // Not enough space in output.
...
ssize_t Base64Decode(const char* src, size_t src_size, uint8_t* dst,
  ...
  if (dst_size < min_dst_size) return -1;
  ...
  while (rd < end) {
    ...
    for (uint32_t j = 0; j < 4; j++) {
      ...
      d[j] = DecodeChar(s[j]);
      if (d[j] == kX) return -1;  // Invalid input char.
    }
